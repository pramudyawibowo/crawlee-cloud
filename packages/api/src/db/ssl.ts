/**
 * Database SSL configuration resolver.
 ...
 *
 * Rules:
 * 1. DB_SSL env var ('false', '0', 'off', 'disable') explicitly disables SSL.
 * 2. DB_SSL env var ('true', '1', 'on', 'require') explicitly enables SSL ({ rejectUnauthorized: false }).
 * 3. sslmode query param in connection URL:
 *    - 'disable', 'allow' -> returns false (SSL disabled, required for non-SSL PgBouncer)
 *    - 'require', 'prefer', 'verify-ca', 'verify-full', 'no-verify' -> returns { rejectUnauthorized: false }
 * 4. In production (NODE_ENV=production):
 *    - defaults to { rejectUnauthorized: false } for external public hosts
 *    - leaves SSL disabled for internal hosts, private IPs, and standard PgBouncer port 6432
 */
